...
embeddings.py
-------------
Fase 1 del sistema de segmentación óptima de contenido.

Dado un texto dividido en oraciones, calcula embeddings semánticos,
mide la similitud coseno entre oraciones adyacentes y produce una
lista ordenada de posiciones candidatas a corte (de más a menos
prometedora). Esta lista guía la inicialización del Simulated
Annealing en la Fase 2.

A diferencia de un filtro estricto, aquí se devuelven TODAS las
posiciones posibles ordenadas por prioridad — garantizando recall
del 100% mientras se aprovecha la información semántica.
...
Uso como módulo:
    from embeddings import get_ranked_candidates, get_initial_cuts
...
def evaluate_recall(
    ...
    """
    Calcula recall top-K y top-2K promedio sobre todo el dataset.
    """
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------
    # Detalle de las 3 primeras instancias de cada dataset
    # -----------------------------------------------------------------------
    for ds_name, dataset in datasets.items():
        print(f"\n{'='*60}")
